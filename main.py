...
def test(test_loader, model, DEVICE, criterion, plt=True):
    with torch.no_grad():
        model.eval()
        total_loss = 0
        n_batches = 0
    ...
    if plt == True:
        ...
        #mds(feats, trgs, save_dir=plot_dir_name + args.model_name + 'mds.png')
...
if __name__ == '__main__':
    ...
    for r in range(args.rep):
        ...
        test_loss = test(test_loader, model_test, DEVICE, criterion, plt=True)
